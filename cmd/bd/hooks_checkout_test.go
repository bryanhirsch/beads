...
func TestIsRebaseInProgress(t *testing.T) {
	...
	initGitRepo := func(t *testing.T) string {
		t.Helper()
		dir := t.TempDir()
...
func TestCheckBeadsRefSyncResetCorrectness(t *testing.T) {
	...
	if _, err := s.AddIssueComment(ctx, "ttravel-aaa", "test", "Comment on A"); err != nil {
	...
	if _, err := s.AddIssueComment(ctx, "ttravel-bbb", "test", "Comment on B"); err != nil {
	...
	// Get the store's actual branch (may be a test branch, not "main")
	storeBranch, err := s.CurrentBranch(ctx)
	if err != nil {
	...
	writeTestBeadsRefs(t, beadsDir, storeBranch, checkpointHash)
	...
	if !slices.Contains(labelsA, "phase1") {
	...
	if refBranch != storeBranch {
		t.Errorf("ref branch = %q, want %q", refBranch, storeBranch)
	}

	// === Reset forward: simulate git checkout back to latest ===
	writeTestBeadsRefs(t, beadsDir, storeBranch, latestHash)
	checkBeadsRefSync(ctx, s)
	...
	if !slices.Contains(labelsA2, "phase1") {
		t.Errorf("round-trip: issue A labels = %v, missing %q", labelsA2, "phase1")
	}
	if !slices.Contains(labelsB2, "phase2") {
...
func TestCheckBeadsRefSyncInSync(t *testing.T) {
	...
	writeTestBeadsRefs(t, beadsDir, branch, currentHash)
